...
class PersonalTransactionsController < ApplicationController
  ...
  def create
    ...
    if @transaction.save
      redirect_to personal_ledger_personal_transactions_path(personal_ledger_id: params[:personal_ledger_id]),
  ...
  def update
    ...
    if @transaction.update(new_params)
      redirect_to personal_ledger_personal_transactions_path(personal_ledger_id: params[:personal_ledger_id]),
                  notice: 'Transaction Successfully Edited'
    else
      render :new
    end
  end
  ...
  def destroy
    ...
    redirect_to personal_ledger_personal_transactions_path(personal_ledger_id: params[:personal_ledger_id]),
                notice: 'Transaction Successfully Deleted'
  end
  ...
  def approval
    ...
    redirect_to personal_ledger_personal_transactions_path(personal_ledger_id: params[:personal_ledger_id]),
                notice: 'Approval recored successfully'
  end
  ...
  def rejection
    ...
    transaction.update(status: :rejected, updated_by: @user.id, determined_by: @user.id,
                       determined_at: Time.zone.now)
    redirect_to personal_ledger_personal_transactions_path(personal_ledger_id: params[:personal_ledger_id]),
                notice: 'Rejection recored successfully'
  end
